...
export default function DashboardOverview() {
  ...
  const stats = [
    { label: 'Active Trips', value: trips.filter(t => t.status === 'active').length, icon: MapPin, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' },
    { label: 'Planned Trips', value: trips.filter(t => t.status === 'planning').length, icon: Calendar, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' },
    { label: 'Total Travelers', value: trips.reduce((sum, trip) => sum + trip.participants, 0), icon: Users, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' },
    { label: 'Total Budget', value: `$${trips.reduce((sum, trip) => sum + (trip.budget?.total || 0), 0).toLocaleString()}`, icon: DollarSign, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-900/20' },
  ];

  const upcomingTasks = [
    { id: 1, task: 'Book flight tickets for Tokyo', trip: 'Tokyo Adventure', due: '2024-02-15', priority: 'high' },
    { id: 2, task: 'Reserve hotel in Shibuya', trip: 'Tokyo Adventure', due: '2024-02-20', priority: 'medium' },
    { id: 3, task: 'Apply for visa', trip: 'European Explorer', due: '2024-03-01', priority: 'high' },
    { id: 4, task: 'Book restaurant reservations', trip: 'Tokyo Adventure', due: '2024-03-10', priority: 'low' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {currentTrip && (
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    currentTrip.status === 'planning' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' :
                    currentTrip.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400' :
            ...
            {/* Budget Overview */}
            {currentTrip.budget && (
              <BudgetOverview budget={currentTrip.budget} />
            )}

            {/* Group Trip Manager */}
            {currentTrip.isGroupTrip && currentTrip.groupMembers && (
              <GroupTripManager 
                groupMembers={currentTrip.groupMembers} 
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-3">
              {upcomingTasks.slice(0, 4).map((task) => (
                <div key={task.id} className="flex items-start space-x-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors">
